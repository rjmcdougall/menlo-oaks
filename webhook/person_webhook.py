"""
Person detection webhook handler for UniFi Protect.
Processes person detection alarm payloads, stores records to BigQuery,
...
PERSON_DETECTION_TYPES = {"person", "person_detected", "person_known", "person_unknown", "person_of_interest"}
PERSON_TABLE = "person_detections"
...
class PersonDetectionHandler:
    """Handles person detection webhook events."""
    ...
    def _table_ref(self):
        return self.bq.dataset(self.dataset_id).table(PERSON_TABLE)

    def _ensure_table_exists(self):
        ref = self._table_ref()
        try:
            self.bq.get_table(ref)
            logger.info(f"Table {PERSON_TABLE} exists")
        except Forbidden:
            logger.info(f"No permission to inspect table {PERSON_TABLE}, assuming it exists")
        except NotFound:
            logger.info(f"Creating table {PERSON_TABLE}")
            ...
            logger.info(f"Created table {PERSON_TABLE}")

    def extract_detections(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Extract person detection records from a UniFi Protect alarm payload."""
        ...
        logger.info(f"Person webhook trigger keys: {all_keys}")

        detections = []
        for trigger in triggers:
            key = trigger.get("key", "")
            if key not in PERSON_DETECTION_TYPES:
    ...
    def _is_active_hours(self) -> bool:
        """Return True if current Pacific time is between 11pm and 5:30am (overnight window)."""
        now_pt = datetime.now(ZoneInfo("America/Los_Angeles"))
        hour, minute = now_pt.hour, now_pt.minute
        return hour >= 23 or hour < 5 or (hour == 5 and minute < 30)

    def process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Process a person detection webhook payload."""
        alert_enabled = self._is_active_hours()
        if not alert_enabled:
            logger.info("Person detection Telegram alert suppressed: outside active hours (7pm–6am Pacific)")

        detections = self.extract_detections(payload)

        if not detections:
            logger.info("No person detection triggers found in payload")
        ...
        for detection in detections:
            try:
                ...
                logger.info(
                    f"Stored person detection: {detection['detection_type']} "
                    f"camera={detection['camera_name']} record_id={record_id}"
                )

                # Send Telegram alert (only during active hours)
                if self.telegram_client and alert_enabled:
